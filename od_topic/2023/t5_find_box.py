...
"""
思路：先遍历数组，用sum_list存储每个节点的前缀和， 注意：这个前缀和需要包含当前节点
然后再逐个判断是否符合题意， sum_list[i] - grid[i] == sum_list[-1] - sum_list[i]即为宝箱
"""
...
def main():
    ...
    for val in box_list:
        if not sum_list:
            sum_list.append(val)
            continue
        sum_list.append(sum_list[-1] + val)

    for idx, val in enumerate(box_list):
        if sum_list[idx] - val == sum_list[-1] - sum_list[idx]:
            print(idx)
            return
    print(-1)
